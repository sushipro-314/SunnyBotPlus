...
parsers = (CommandParser(), OtherParser())
...
class LavalinkVoiceClient(discord.VoiceProtocol):
    ...
    def __init__(self, client: discord.Client, channel: discord.abc.Connectable):
        ...
        if not hasattr(self.client, 'lavalink'):
            ...
            for node in parsers[1].config["uris"]["lavalink"]:
                logging.info("attempting to connect to " + node["host"] + " from config")
                self.client.lavalink.add_node(host=node["host"], port=int(node["port"]), password=node["pass"],
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):
            ...
            for node in parsers[1].config["uris"]["lavalink"]:
                logging.info("attempting to connect to " + node["host"] + " from config")
                bot.lavalink.add_node(host=node["host"], port=int(node["port"]), password=node["pass"],
                                            region=node["region"], name=node["name"])
    ...
    async def play(self, ctx, query: str, limit: int):
        ...
        # Contains a list of field-values for generating an embed
        indexed_track_fields = []
        # Tells us how many songs their are before we should stop
        duration = 0
        # Max limit for how many songs you can add before the bot stops adding them
        # The max limit for songs can be set in config
        if (limit) and limit <= parsers[1].config["max_song_count"]:
            max_limit = limit
        elif parsers[1].config["max_song_count"]:
            # If the config options exists
            max_limit = parsers[1].config["max_song_count"]
        else:
