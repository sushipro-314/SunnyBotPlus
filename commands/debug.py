...
class Debug(commands.Cog):
    ...
    async def on_command_error(self, ctx, error: discord.DiscordException):
        ...
        error_id = str(hash(ctx.guild.name)) + "-!!-" + str(ctx.message.id) + "-!-" + str(ctx.message.author.id)
        ...
        await ctx.send("An issue occurred. Error ID: " + error_id, embed=await embed_system.generate_embed(title="An unknown issue occurred", desc=str(error.args)))
